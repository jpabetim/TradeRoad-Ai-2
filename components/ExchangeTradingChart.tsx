...
import { getBinanceCandles, getBingXCandles, adaptCandlesToLightweightCharts } from '../services/exchangeService';

interface ExchangeTradingChartProps {
...
const ExchangeTradingChart: React.FC<ExchangeTradingChartProps> = ({
  exchange = 'binance',
  symbol = 'BTCUSDT',
  interval = '1d',
  height = 400,
  ...
  const fetchData = async () => {
    try {
      ...
      // Obtener datos según el exchange seleccionado
      let candles;
      if (exchange === 'binance') {
        candles = await getBinanceCandles(symbol, interval, 100);
      } else if (exchange === 'bingx') {
        candles = await getBingXCandles(symbol, interval, 100);
      } else {
        throw new Error('Exchange no soportado');
      }
      
      if (!candles || candles.length === 0) {
        setError(`No se pudieron obtener datos para ${symbol} en ${exchange}`);
        setIsLoading(false);
        return;
      }
      
      // Adaptar los datos al formato de Lightweight Charts
      const candleData = adaptCandlesToLightweightCharts(candles);
  ...
  return (
    <div className="relative w-full">
      ...
      <div className="mb-4 flex justify-between items-center">
        <h2 className="text-xl font-bold">{symbol} - {exchange === 'binance' ? 'Binance' : 'BingX'}</h2>
...
export default ExchangeTradingChart;
